...
var (
	...
	p.Iterations_used          int
	p.Iterations_of_event      int
	b                        *bomb
	...
	squareRowCM                    int     //This is an int 1 through p.MaxX representing how many rows of squares there are
	squareColCM                    int     //This is an int 1 through p.MaxY representing how many columns of squares there are
	...
	driftFile    *os.File
	nodeFile     *os.File
	p.PositionFile *os.File
	statsFile    *os.File

	r.Point_list []cps.Tuple

	r.Point_list2 [][]bool

	r.Point_dict map[cps.Tuple]bool

	r.Square_list []RoutingSquare

	r.Border_dict map[int][]int

	r.Node_tables []map[cps.Tuple]float64

	possible_paths [][]int

	stim_list map[int]cps.Tuple
