...
if __name__ == '__main__':
    ...
    aloneRCThreshold = ["-aloneThreshold=%f -aloneOrClusterRatio=true" % f for f in [0.025, 0.05, 0.1]]
    clusterRCThreshold = ["-aloneThreshold=%f -aloneOrClusterRatio=false" % f for f in [0.2, 0.25, 0.3]]
    aloneThreshold = aloneRCThreshold + clusterRCThreshold
    aloneClusterSearch = ["-aloneClusterSearch=%s" % s for s in ['true', 'false']]
    reclusterPeriod = ["-reclusterPeriod=%d" % d for d in [100.0, 200.0]]
    localRecluster = ["-localRecluster=%d -expansiveRatio=%f" % (d, f) for (d, f) in [(0,0), (1,0), (2,0), (3, 0.33), (3, 0.67)]]
    ...
    CHTimeThresh = ["-CHTimeThresh=%d" % d for d in [450, 900]]
    CHBatteryDropThresh = ["-CHBatteryDropThresh=%f" % f for f in [0.3, 0.4]]
    smallImprovementRatio = ["-smallImprovementRatio=%f" % f for f in [0.33]]
    largeImprovement = ["-largeImprovement=%f" % f for f in [0.7]]
    GRIncrement = ["-GRIncrement=%f" % f for f in [1.75]]
    GRDecrement = ["-GRDecrement=%f" % f for f in [0.7]]
    disableGRThresh = ["-disableGRThresh=%f" % f for f in [0.7]]
    disableCSThresh = ["-disableCSThresh=%f" % f for f in [0]]
    maxClusterHeads = ["-maxClusterHeads=%d" % d for d in [1]]
    serverReadyThresh = ["-serverReadyThresh=%f" % f for f in [0.99]]
    ...
    globalRecluster3 = (list(itertools.product(*[["-globalRecluster=3"], aloneThreshold, reclusterPeriod, globalReclusterOnOptions])))
    for i in range(len(globalRecluster3)):
        globalRecluster3[i] = ' '.join(globalRecluster3[i])

    globalReclusterOptions = globalRecluster0 + globalRecluster1 + globalRecluster3
    ...
    for r in runs:
        for i in range(factor):
            j = [zz for zz in r]
            j.append("-OutputFileName=/home/simulator/git-simulator/CPS_Simulator/simData/AloneOrCluster_2020-7-27/Log_%d" % x)
