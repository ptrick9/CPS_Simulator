		...
		for _, s := range scheduler.SNodeList {
			...
			if length != len(s.GetRoutePoints()) {
				...
				if bombSquare == sSquare || bdist < 8.0 {
					...
					//Report and calculate run time
					var sumOfTimes int64 = 0
					avgTime := 0.0
					for i := 0; i < p.Iterations_used; i++ {
						sumOfTimes += runTimes[i]
					}
					avgTime = float64(sumOfTimes) / float64(p.Iterations_used)
					fmt.Printf("\nAverage run time: %vus\n", avgTime / 1000)
