...
var (
	...
	wallNodeList []cps.WallNodes
	nodeList     []cps.NodeImpl
	...
	numAtt         int
	attractions    []*cps.Attraction
	bombSquare     *cps.Square
	xLoc           int
	yLoc           int
...
func main() {
	...
	p.MaxX = getDashedInput("maxX")
	p.MaxY = getDashedInput("maxY")
	p.BombX = getDashedInput("bombX")
	p.BombY = getDashedInput("bombY")
	...
	p.XDiv = maxX / squareCol
	p.YDiv = maxY / squareRow
	...
	squareCapacity = int(math.Pow(float64(p.XDiv), 2))
	fmt.Println("xDiv is ", xDiv, " yDiv is ", p.YDiv, " square capacity is ", squareCapacity)
...
func getFlags() {
	...
	flag.IntVar(&p.SuperNodeType, "superNodeType", 6, "the type of super node used in the simulator")
	...
	flag.IntVar(&p.SquareRowCM, "squareRow", 100, "Number of rows of grid squares, 1 through maxX")
	flag.IntVar(&p.SquareColCM, "squareCol", 100, "Number of columns of grid squares, 1 through maxY")
