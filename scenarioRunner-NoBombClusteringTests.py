...
if __name__ == '__main__':
    ...
    globalRecluster = ["-globalRecluster=%d" % d for d in [1]]
    reclusterThreshold = ["-reclusterThreshold=%f" % f for f in [0.01, 0.025, 0.05, 0.1]]
    reclusterPeriod = ["-reclusterPeriod=%d" % d for d in [300.0]]
    localRecluster = ["-localRecluster=%d" % d for d in [0, 2]]
    #expansiveLocal = ["-localRecluster=%d" % (d+3) for d in [0, 60, 120, 180]]
    #localRecluster += expansiveLocal
    ...
    clusterSearchThresh = ["-clusterSearchThresh=%d" % d for d in [0]]
    #initClusterTime = ["-initClusterTime=%d" % d for d in [0, 200, 600]]
    CHTimeThresh = ["-CHTimeThresh=%d" % d for d in [300]]
    CHBatteryDropThresh = ["-CHBatteryDropThresh=%f" % f for f in [0.3]]
    smallImprovement = ["-smallImprovement=%f" % f for f in [0.2]]
    largeImprovement = ["-largeImprovement=%f" % f for f in [0.6]]
    GRIncrement = ["-GRIncrement=%f" % f for f in [1.5]]
    GRDecrement = ["-GRDecrement=%f" % f for f in [0.7]]
    maxClusterHeads = ["-maxClusterHeads=%d" % d for d in [1]]

    globalReclusterOptions = (list(itertools.product(*[globalRecluster, reclusterThreshold, reclusterPeriod, smallImprovement,
                                                        largeImprovement, GRIncrement, GRDecrement])))
    for i in range(len(globalReclusterOptions)):
        globalReclusterOptions[i] = ' '.join(globalReclusterOptions[i])
    globalReclusterOptions += ["-globalRecluster=0 -reclusterThreshold=0.15 -reclusterPeriod=300.0"]
    ...
    clusteringOptions = (list(itertools.product(*[clusteringOn, clusterMax, clusterMin, degreeWeight, batteryWeight,
                                                globalReclusterOptions, localReclusterOptions, BTLoss, clusterSearchThresh,
                                                maxClusterHeads])))
    for i in range(len(clusteringOptions)):
        clusteringOptions[i] = ' '.join(clusteringOptions[i])
    #clusteringOptions += ["-clusteringOn=false -reclusterThreshold=0.15 -reclusterPeriod=90 -bluetoothLossPercentage=0.000005"]
    ...
    for r in runs:
        for i in range(factor):
            j = [zz for zz in r]
            j.append("-OutputFileName=/home/simulator/git-simulator/CPS_Simulator/simData/clusteringTest2020-7-15/Log_%d" % x)
